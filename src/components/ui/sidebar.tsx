...
function SidebarProvider({
  ...
  const [_open, _setOpen] = React.useState(() => {
    const isOpen = localStorage.getItem(SIDEBAR_LOCAL_STORAGE_KEY);
    if (isOpen !== null) {
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      console.log("assign to cookie", openState);

      // This sets the cookie to keep the sidebar state.

      localStorage.setItem(SIDEBAR_LOCAL_STORAGE_KEY, openState.toString());
    },
    [setOpenProp, open]
  );
...
function SidebarTrigger({
  ...
  return (
    ...
    >
      {open ? (
        <PanelRightOpen className="size-5.5" size={22} aria-hidden="true" />
      ) : (
        <PanelRightClose className="size-5.5" size={22} aria-hidden="true" />
      )}
